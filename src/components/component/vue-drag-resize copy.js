...
export default {
    ...
    props: {
        ...
        //     type: Array,
        //     default() {
        //         return ['tl', 'tm', 'tr', 'mr', 'br', 'bm', 'bl', 'ml', 'trmv'];
        //     },
        // },
        sticks: {
            type: Array,
            default() {
                return ['tm', 'mr', 'bm', 'ml', 'trmv'];
            },
        },
        axis: {
            type: String,
    ...
    methods: {
        ...
        move(ev) {
            console.log('vue-drag-resize move') 
            // if (!this.stickDrag && !this.bodyDrag) {
            //     return;
            // }
            if (!this.stickDrag && !this.bodyDrag && !this.stickBodyDrag) {
                return;
            }
        ...
        up(ev) {
            console.log('vue-drag-resize up')
            if (this.stickDrag) {
                this.stickUp(ev);
    ...
    computed: {
        ...
        //         if (stick === 'trmv') {
        //             stickStyle[styleMapping.y[stick[0]]] = `${this.stickSize / this.parentScaleX / -2 - this.stickSize/2}px`;
        //             stickStyle[styleMapping.x[stick[1]]] = `${this.stickSize / this.parentScaleX / -2 + this.stickSize/2}px`;
        //         }
        //         return stickStyle;
        //     };
        // },
        vdrStick() {
            return (stick) => {
                if (stick==='trmv') {
                    this.stickToolbarSizeW = 100
                    this.stickToolbarSizeH = 20
                    const stickStyle = {
                        width: `${this.stickToolbarSizeW / this.parentScaleX}px`,
                        height: `${this.stickToolbarSizeH / this.parentScaleY}px`,
                    };
                    stickStyle[styleMapping.y[stick[0]]] = `${this.stickToolbarSizeH / this.parentScaleX / -2}px`;
                    stickStyle[styleMapping.x[stick[1]]] = `${this.stickToolbarSizeW / this.parentScaleX / -2}px`;
                    if (stick === 'trmv') {
                        stickStyle[styleMapping.y[stick[0]]] = `${this.stickToolbarSizeH / this.parentScaleX / -2 - this.stickToolbarSizeH/2}px`;
                        stickStyle[styleMapping.x[stick[1]]] = `${this.stickToolbarSizeW / this.parentScaleX / -2 + this.stickToolbarSizeW/2}px`;
                    }
                    return stickStyle;
                } else {
                    const stickStyle = {
        ...
        height() {
            return this.h === 'auto' ? 'auto' : this.parentHeight - this.top - this.bottom;
        },
    ...
    watch: {
        ...
        x: {
            handler(newVal, oldVal) {
                ...
                //     return;
                // }
                if (this.stickDrag || this.stickBodyDrag || this.bodyDrag || (newVal === this.left)) {
                    return;
                }
        ...
        y: {
            handler(newVal, oldVal) {
                ...
                //     return;
                // }
                if (this.stickDrag || this.stickBodyDrag || this.bodyDrag || (newVal === this.top)) {
                    return;
                }
        ...
        w: {
            handler(newVal, oldVal) {
                ...
                //     return;
                // }
                if (this.stickDrag || this.stickBodyDrag || this.bodyDrag || (newVal === this.width)) {
                    return;
                }
        ...
        h: {
            handler(newVal, oldVal) {
                ...
                //     return;
                // }
                if (this.stickDrag || this.stickBodyDrag || this.bodyDrag || (newVal === this.height)) {
                    return;
                }
